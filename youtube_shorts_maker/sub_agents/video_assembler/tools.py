...
async def assemble_video(tool_context: ToolContext) -> str:
    ...
    """
    temp_files = []  # 임시 파일 리스트 생성

    try:
        # content_planner_output과 scene 추출
        ...
        # 존재하는 아티팩트 리스트 나열 - 이미지와 나래이션들
        existing_artifacts = await tool_context.list_artifacts()

        # 이미지 파일과 오디오 파일 리스트 생성
        image_files = []
        audio_files = []
